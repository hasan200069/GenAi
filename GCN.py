...
def get_spatial_relation(box1, box2):
    ...
    def generate_scene_graph(self, image_path):
        try:
            ...
            for i in range(len(scene_graph['objects'])):
                for j in range(i + 1, len(scene_graph['objects'])):
                    subject = scene_graph['objects'][i]
                    obj = scene_graph['objects'][j]
                    relation = get_spatial_relation(subject['bbox'], obj['bbox'])
            ...
            print(f"An error occurred during scene graph generation: {e}")
            return None



def train_model(model, dataloader, epochs=10, plot_interval=5, device='cpu', save_interval=10, save_path='model.pth'):
    ...
    for epoch in range(epochs):
        ...
        for step, data in enumerate(dataloader):
            ...
            if (step + 1) % plot_interval == 0:
                ...
                print(f"Model saved at epoch {step+1}.")
...
if __name__ == "__main__":
    ...
    # Change this to use raw string literal or double backslashes
    image_path = r"D:\Project\images\2.jpg"



    scene_graph = model.generate_scene_graph(image_path)
    print(scene_graph)
